...
async function handleGet(request: Request): Promise<Response> {
  ...
  if (!user) {
    return jsonWithCors(request, { error: 'Unauthorized.' }, { status: 401, headers: NO_STORE_HEADERS }, ALLOWED_METHODS);
  }
  ...
  try {
    ...
    if (!isAdmin) {
      return jsonWithCors(request, { error: 'Forbidden.' }, { status: 403, headers: NO_STORE_HEADERS }, ALLOWED_METHODS);
    }
    ...
    return jsonWithCors(request, { entries }, { headers: NO_STORE_HEADERS }, ALLOWED_METHODS);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to load password reset audit.';
    return jsonWithCors(request, { error: message }, { status: 500, headers: NO_STORE_HEADERS }, ALLOWED_METHODS);
  }
}
