...
function getApnsConfig() {
  return {
    ...
    topic: (process.env.APPLE_PUSH_TOPIC ?? process.env.IOS_BUNDLE_ID ?? 'com.hiitsme.app').trim(),
  };
}
...
async function getApnsJwt() {
  ...
  const token = await new SignJWT({})
    ...
    .setIssuedAt()
    .sign(privateKey);
  cachedJwt = { token, expiresAtMs: Date.now() + 50 * 60 * 1000 };
  return token;
}
...
function buildApnsPayload(input: {
  ...
  return {
    aps: { alert: { title: input.senderName, body: input.messagePreview }, sound: PUSH_SOUND },
...
async function sendApnsNotificationViaHost(
  ...
  return await new Promise<{ status: number; body: string }>((resolve, reject) => {
    ...
    client.on('error', (error) => { client.close(); reject(error); });
    ...
    req.on('end', () => { client.close(); resolve({ status, body: responseBody }); });
...
function resolveApnsHosts(pushEnvironment: ApnsEnvironment | null) {
  if (pushEnvironment === 'sandbox') return [APNS_SANDBOX_HOST];
  if (pushEnvironment === 'production') return [APNS_PRODUCTION_HOST];
...
async function dispatchToRecipients(input: {
  ...
  if (input.recipientIds.length === 0) return { delivered: 0, attempted: 0 };
  ...
  if (privacyResponse.error) throw new Error(privacyResponse.error.message);
  ...
  for (const row of tokenRows ?? []) {
    ...
    if (!existing.some((entry) => entry.token === row.token)) {
      existing.push({ token: row.token, pushEnvironment: normalizePushEnvironment(row.push_environment) });
    ...
    for (const tokenEntry of tokens) {
      ...
      for (const host of hosts) {
        try {
          ...
          if (!shouldPruneTokenForReason(reason)) allResponsesPrunable = false;
        } catch (error) {
...
async function dispatchDirectMessagePush(
  ...
  if (!message || message.sender_id !== actorUserId) throw new Error('Message not found.');
...
async function dispatchRoomMessagePush(
  admin: ReturnType<typeof createSupabaseAdminClient>,
  actorUserId: string,
  roomMessageId: string,
) {
  ...
  if (!message || message.sender_id !== actorUserId) throw new Error('Room message not found.');
...
async function handlePost(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (body.kind === 'room') {
      ...
      if (!roomMessageId) {
        return jsonWithCors(request, { error: 'roomMessageId is required.' }, { status: 400 }, PUSH_DISPATCH_METHODS);
      }
    ...
    if (body.kind === 'buddy_request' || body.kind === 'buddy_accept') {
      ...
      const result = await dispatchBuddyRelationshipPush({ admin, actorUserId: user.id, buddyId, kind: body.kind });
      return jsonWithCors(request, { ok: true, ...result }, undefined, PUSH_DISPATCH_METHODS);
    }
...
export default async function handler(request: Request): Promise<Response> {
  if (request.method === 'OPTIONS') return createCorsPreflightResponse(request, PUSH_DISPATCH_METHODS);
