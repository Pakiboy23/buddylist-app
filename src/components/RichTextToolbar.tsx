...
export default function RichTextToolbar({ value, onChange }: RichTextToolbarProps) {
  ...
  return (
    <div className="ui-toolbar-surface rounded-2xl px-3 py-3">
            ...
            return (
              <button
                ...
                className={`ui-focus-ring shrink-0 rounded-full border px-3 py-1.5 text-[12px] font-semibold transition ${
                  isActive
                    ? 'border-[#E8A23A]/40 bg-[#E8A23A] text-white shadow-[0_10px_20px_rgba(232,162,58,0.24)]'
                    : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-[#0F1424]/65 dark:text-slate-200 dark:hover:bg-[#0F1424]'
                }`}
      ...
      <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white/80 px-3 py-2 dark:border-slate-700 dark:bg-[#0F1424]/55">
