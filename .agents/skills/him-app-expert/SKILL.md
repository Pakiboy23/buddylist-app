...
- **Rooms v2 RLS:** direct INSERT on `room_memberships` recurses. Preview join/leave go through the SECURITY DEFINER RPCs. Buddy-pull invites go through `rooms-invite` (service-role upsert), not those RPCs.
...
- **Presence:** chips from `derivePresenceState` (`src/lib/presence.ts`), not leftover `users.status`. Browse uses a 24h `last_active_at` window. `show_online_status` (default true) hides chips and last-active from others; away-message text can stay. Independent of `discoverable`. RLS does not hide columns — use `user_presence_directory` or the app-layer mask. Global people lookups must call `applyDiscoverablePeopleGate`. Runbook: `docs/presence.md`.
- **Invites:** `rooms-invite` requires an accepted buddy. There are no shareable invite links. `/join/:inviteCode` discards the code. CORS must allow `apikey` **on the deployed function** (`supabase functions deploy rooms-invite`) or Safari shows "Load failed". Do not invent viral links. Runbook: `docs/rooms-invite.md`.
