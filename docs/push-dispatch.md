...
- `maybePromptForPushAfterFriendshipAction` after a completed `buddy_accepted`, `first_dm_sent`, or `first_room_message`

Friendship-action callers: `src/lib/buddyRequest.ts` (both Accept buttons go through `acceptIncomingBuddyRequest`) and `src/lib/messageIdempotency.ts` (`first_dm_sent` on every successful DM insert; `first_room_message` on every successful room insert). Lost-ack 23505 reconciles do not re-prompt.
