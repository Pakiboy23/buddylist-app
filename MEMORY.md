...
- **2.4 SHIPPED. The train is closed.** Live App Store is Ready for Sale on marketing version **2.4** / build **453**. Overnight uploads of 2.4 builds 454 and 457 failed with `ITMS-90186` (train 2.4 closed) and `ITMS-90062` (`CFBundleShortVersionString` must be higher than the approved 2.4). Do **not** produce another 2.4 binary. `MARKETING_VERSION` is now **2.5**; any further work ships as 2.5. Xcode Cloud sets the build number itself (it reached 457), so `CURRENT_PROJECT_VERSION` in the project is inert.
