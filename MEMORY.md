...
## Project Overview
H.I.M. (`hiitsme`) — retro AIM-style mobile-first messaging app. Vite + React 19 + React Router v7, Vercel (web), Capacitor 8 (iOS; Android dropped in #147), Supabase backend. **Shipped.** v2.2 (build 314) is live on the App Store; repo is on 2.3. ~128 accounts. Now in a growth flight, not a build sprint.
...
- **2.5 train is closed.** App Store Connect rejected build **464** on marketing version **2.5** with `ITMS-90186` (train 2.5 closed for new submissions) and `ITMS-90062` (`CFBundleShortVersionString` must be higher than the approved 2.5). Do **not** produce another 2.5 binary. `MARKETING_VERSION` is now **2.6** and `CURRENT_PROJECT_VERSION` is **465** (one past rejected build 464). Any further App Store submission ships as 2.6 / 465+.
...
## Blockers
- **iOS renders the React app; there is no native UI.** `f6fbad5` (30 Aug) set `HiItsMeShell.isAvailable` to `false` and stopped installing the native chrome, so React owns every pixel on the phone. The SwiftUI overlay, the UIKit chrome and the whole milestone-one bridge were then deleted. **A web change reaches iOS only by rebuilding: `npm run build && npm run ios:sync`, then commit `dist/` + `ios/App/App/public`.** Do not reimplement screens natively — that is exactly the loop that cost 29–30 Aug: the hand-written SwiftUI buddy list could never catch up with the React one, and four rounds of porting still did not match.
...
- `CURRENT_PROJECT_VERSION` is **465** so the next archive is past rejected build 464 on the closed 2.5 train. Xcode Cloud may still assign a higher build number from App Store Connect; that number must stay above 464.
