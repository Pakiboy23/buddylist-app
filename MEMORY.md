...
- **Two Xcode Cloud workflows are misconfigured (console-side, 29 Aug).** `App | Buddy` targets a scheme named `App` that does not exist — the scheme is `HIM`; it fails every run. `H.I.M. | H.I.M. | Test - iOS` targets `AparajitaCapacitorBiometricAuth`, a vendored SPM dependency with no test bundles. Both fail regardless of anything in the repo. Delete or repoint them.
