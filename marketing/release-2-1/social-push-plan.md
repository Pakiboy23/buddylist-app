...
| **Knock** ("gentle low-pressure hello") | **YES** (corrected — see §0a) | `handleSendKnockToBuddy` in `src/app/hi-its-me/page.tsx` + `src/components/ChatWindow.tsx`; migrations `20260722114338` / `20260722150347`; `enforce_knock_rules()` live in prod. 62 refs on `main`. | **Green to build content.** Buddies-only "👋 Knock", 10-min cooldown, screen-recordable in the native build. Needs a claims-register entry like every 2.1 feature. |
| **Follow / connection system** (a *second* low-pressure gesture) | **IN CODE, LIVE-STATUS UNVERIFIED** | `user_connections` table, RPCs `get_connection_status` / `can_add_from_room`, `useConnectionStatus.ts`; `BuddyProfileSheet.tsx` only activates it when a `currentUserId` prop is passed | **Verify it's enabled in the shipped build before any Follow content** (§5.2). Treat as a stretch beat, not a launch pillar. |

**Corrected finding (see §0a):** Both **Knock** and **Follow** ship and are distinct. Knock is a buddies-only "👋 wants to talk" ping (real, live in prod, screen-recordable); Follow is a one-way connection that gates presence/away-message visibility. The founder's "gentle, no-obligation hello" is served by *both* — Knock is the direct ping, Follow is the ambient presence gesture. Build Knock content now; gate Follow content on the §5.2 live-render check. Neither may be framed as an AIM/AOL "nudge™" (brand brief §7).
...
- **Hard rule inherited from `tiktok.md` §1:** real UI only. Every 2.1 demo screen-records a **shipped button.** Knock qualifies (it's live — §0a) and gets content; Follow is gated on the §5.2 live-render verification.
...
- **Knock — real, market it honestly (corrected, §0a).** Buddies-only "👋 Knock", 10-min cooldown, writes a fixed-payload message row (inherits content filter + block guard + report sheet). Screen-record the real button only. **Never frame it as an AIM/AOL "nudge™"** or any legacy-IM trademark (brand brief §7); never imply it reaches non-buddies. Needs a claims-register entry before public copy (§5.1).
