...
The `android:*` scripts were removed. The `android/` directory they operated
on was deleted in #147 — do not restore either.
...
(`buddy_accepted`, `first_dm_sent`, `first_room_message`). iOS permission
state is the source of truth for whether this install has been asked;
`him.pushPrompt.askedAt` is a 7-day cooldown, not a permanent veto. Do not
skip the ask because token rows exist — they outlive the install.
